    ...
    <nav className="fixed top-0 w-full z-50 transition-all duration-300" 
      ...
      <div className="flex items-center justify-between px-4 py-2">
        ...
        <div className="flex items-center gap-4">
          ...
          <form onSubmit={handleSearch} className="flex items-center bg-netflix-black/95 rounded-md border border-white/10 px-2">
            <Search className="h-3 w-3 text-white mr-2" />
            <input
              type="text"
              placeholder="Search..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-32 bg-transparent text-sm text-white border-none outline-none placeholder:text-gray-400 h-8"
            />
          </form>
        </div>
      </div>
